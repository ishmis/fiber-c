...
int prog(int __attribute__((unused)) argc, char **argv) {
  ...
  while (p < max_primes) {
    ...
    for (uint32_t j = 0; j < p; j++) {
      divisible = (bool)(intptr_t)fiber_resume_with(
          filters[j], (void *)(intptr_t)i, &status);
    ...
    if (!divisible) {
      ...
      (void)fiber_resume_with(filter_fiber, (void *)(intptr_t)i, &status);
      assert(status == FIBER_YIELD);
  ...
  for (uint32_t i = 0; i < p; i++) {
    (void)fiber_resume_with(filters[i], (void *)(intptr_t)0, &status);
